...
public class MainTick {
	private static Logger logger = LoggerFactory.getLogger(MainTick.class);
	...
	public MainTick() {
		...
		logger.info("Data Loading ("
				...
				+ decimalFormat.format(new Double(tickGenerator.getCount() * 1000)
						/ (new Double(tickTotal.get()).doubleValue())) + " a sec)");
	...
	class TimeSeriesTickWriter implements Runnable {
		...
		public void run() {
			...
			while (true) {
				...
				if (timeSeries != null) {
					try {
						List<TickData> tickDataList = createTimeSeriesList(timeSeries);
